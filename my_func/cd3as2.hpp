...
struct cd3as2{
    ...
    Eigen::ArrayXcd sigma3_iee,sigma3_iee_b1,sigma3_iee_b2;
    Eigen::ArrayXcd sigma3_spm1;
    Eigen::ArrayXcd sigma3_spm2;
    Eigen::ArrayXcd sigma3_3thz;
    Eigen::ArrayXcd sigma3_3f;
...
//thz=2w2-w1
  ...
  w2=tf.omega_p2;
  w3=tf.omega_p2;
  sigma3_iee=sigma3(w1,w2,w3);
//w1=2w2-thz
w1=-tf.omega_thz;
sigma3_iee_b1=sigma3(w1,w2,w2);
//w2=w1+thz-w2
w1=-tf.omega_p2;
sigma3_iee_b2=sigma3(tf.omega_thz,tf.omega_p1,w1);
//the spm is relatively flat in frequency response
  w1=-tf.omega_p1;
  w2=tf.omega_p1;
  w3=tf.omega_p1;
  sigma3_spm1=sigma3(w1,w2,w3);
 // sigma3_spm1=(-9.27e-10+C.II*6.56e-12)*Eigen::ArrayXd::Ones(tf.N_f);//;
  w1=-tf.omega_p2;
  w2=tf.omega_p2;
  w3=tf.omega_p2;
  sigma3_spm2=sigma3(w1,w2,w3);
  //sigma3_spm2=(-1.8552e-09 +C.II*2.6248e-11)*Eigen::ArrayXd::Ones(tf.N_f);//sigma3(w1,w2,w3);
//   w1=tf.omega_thz;
//   w2=tf.omega_thz;
//   w3=tf.omega_thz;
//   sigma3_3thz=sigma3(w1,w2,w3);
//  sigma3_3f=tf.omega_p1;//sigma3(w1,w2,w3);
...
void cd3as2::assign_chi1(T1 &tf){
   ...
   omega_max=tf.omega_1;
   if(tf.omega_1<tf.omega_2)omega_max=tf.omega_2;
   std::tie(n_thz,a_thz)=n(tf.omega_thz);
   std::tie(n_p1,a_p1)=n(tf.omega_p1);
   std::tie(n_p2,a_p2)=n(tf.omega_p2);
