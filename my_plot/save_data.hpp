...
struct save_data{
      ...
      Eigen::ArrayXd thz_spectrum,eff,Energy_cons;
      Eigen::ArrayXd eff_3hg;
      bool cond2,cond3;
      Eigen::ArrayXcd e_ir1,e_ir2,e_thz;
       ...
       if((k%z.N_z_save)==0){
                   ...
                   // my_print(z.N_z_iter);
                    ...
                    if(z.N_z_iter>2){
                            ...
                            cond2=(eff[z.N_z_iter-2]<eff[z.N_z_iter-1]);
                            cond3=(eff[z.N_z_iter-1]>eff[z.N_z_iter]);
                            if(cond2&&cond3){
...
CONST C;
    ...
    if(isnan(eff[j])||isinf(eff[j])||(Energy_cons[j]>Energy_cons[0])){
        ...
        save_array(eff,save_path+b+"eff.txt");
        save_array(Energy_cons,save_path+"energy_cons.txt");
       
        //std::cout<<"is energy conservation breaked:"<<(Energy_cons[j]>Energy_cons[0]);
        exit(0);
    }

    //sum up for THZ 3HG
    int N_start=N_f/2.0+3.0*tf.N_thz_3h-0.5*tf.N_thz_3h;
    Energy_thz=0;
    for (int i=0;i<tf.N_thz_3h;i++){
...
void save_data::save_eff_spec(const std::string& b,T1&mypython){
    ...
    save_array(e_thz,save_path+"thz_e_w.txt");
...
void save_data::save_efield(int N_f,const std::string& b,T1& fftw,T2& mypython){
    ...
    save_array(E_t_final,save_path+b+"et_thz.txt");
